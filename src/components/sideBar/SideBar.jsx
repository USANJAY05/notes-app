...
const SideBar = () => {
  ...
  console.log(toggle)
  return (
    <div className={`w-64 h-full border-r dark:border-r-gray-700 z-50 bg-white dark:bg-black border-b-gray-50 box-border overflow-auto ${!toggle && 'hidden'} ${mobileSize && 'fixed w-full border-none'}`}>
      {notes.length ?(
      notes.map(item =>(
        <div 
          key={item.id} 
        ...
        ><h2 className='p-2 py-3'>{item.content.slice(0,20)+"..."}</h2><hr className='border-gray-200 dark:border-gray-700' /></div>
